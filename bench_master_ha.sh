...
output_log="benchmark_times_ha.log"
...
for num_vars in "${num_vars_values[@]}"; do
  ...
  for ((log_num_workers = 1; log_num_workers <= max_log_num_workers; log_num_workers++)); do
    ...
    output=$(cargo run --release --package cirrus --bin bench_master --features bench-master -- \
      --num-vars "$((1 + log_num_workers))" --log-num-workers "$log_num_workers" --num-threads 1 2>&1)
    ...
    echo "--log-num-vars=$num_vars, --log-num-workers=$log_num_workers, time elapsed: ${time_in_ms}ms" >> "$output_log"

    # Optionally, display the elapsed time for each run
    echo "Time elapsed for --log-num-vars=$num_vars and --log-num-workers=$log_num_workers: ${time_in_ms}ms"
  done
done
