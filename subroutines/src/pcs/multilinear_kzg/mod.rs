...
impl<E: Pairing> PolynomialCommitmentSchemeDistributed<E> for MultilinearKzgPCS<E> {
    ...
    /// Distribute the prover parameter generated by PolynomialCommitmentScheme::trim
    /// into master and worker parameters, both of which are of the same type.
    ///
    /// Originally, the prover parameter contains a list of powers of g, which can be
    /// visually represented as follows:
    ///       powers_of_g = [[0:2^num_vars], [0:2^(num_vars-1)], ..., [0:2^0]]
    ///
    /// The master parameter contains the last `log_num_workers` number of powers of g,
    /// while the worker parameters distribute evenly the remaining powers of g, with
    /// each taking its share of every element in powers_of_g.
    ...
    ///
    /// Arguments:
    /// - `master_prover_param`: the master prover parameter returned by `prover_param_distributed`.
    /// - `handle`: the handle of the multilinear extension to commit to, which is the number of
    ...
    ///
    /// Arguments:
    /// - `worker_prover_param`: the worker prover parameter returned by `prover_param_distributed`.
    ...
    /// Arguments:
    /// - `master_prover_param`: the master prover parameter returned by `prover_param_distributed`.
    /// - `handle`: the handle of the multilinear extension to open, which is the number of variables
    /// of the multilinear extension.
    /// - `point`: the point at which to open the multilinear extension. The length of the point
    /// should be equal to the handle.
    /// - `master_channel`: the channel to communicate with the workers.
    ///
    /// Returns:
    /// - the proof of the opening and the evaluation of the polynomial at the point.
    ...
    /// Open a polynomial in a distributed manner.
    ///
    /// The worker prover only has its share of the multilinear extension, waiting for the master
    /// prover to send the opening point to open the multilinear extension.
    ///
    /// Arguments:
    /// - `worker_prover_param`: the worker prover parameter returned by `prover_param_distributed`.
    ...
    /// Generate a batch proof for multiple openings of polynomials at different points
    ...
    /// - `handle`: the handle of the multilinear extension to open, which is the number of variables
    /// of the multilinear extension.
    /// - `points`: the points at which to open the multilinear extensions. The length of each point
    /// should be equal to the handle.
    ...
    /// Arguments:
    /// - `worker_prover_param`: the worker prover parameter returned by `prover_param_distributed`.
    /// - `polynomials`: the worker's shares of the multilinear extensions to open.
    /// - `worker_channel`: the channel to communicate with the master.
