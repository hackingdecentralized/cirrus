...
def transfer(ip, main_key_path, local_zip_path, commands):
    ...
    try:
        ...
        # Execute each command from the commands list
        for command in commands:
            ...
            # Sleep for a short time between commands to prevent overload
            time.sleep(1)
...
# def run_commands_on_ec2(ip, key_path, commands):
#     """Run a list of commands on an EC2 instance using SSH."""
#     print(f"Connecting to {ip}...")
#     try:
#         # Create an SSH client
#         ssh = paramiko.SSHClient()
...
# Run the setup process on each EC2 instance
for ip in ec2_public_ips:
    transfer_and_setup_repo(ip, key_path, local_zip_path, commands)