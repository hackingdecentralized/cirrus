...
fn main() -> Result<(), HyperPlonkErrors> {
    ...
    let pk_master = HyperPlonkProvingKeyMaster::<E, PCS>::deserialize_compressed(file).unwrap();
    let log_num_workers = pk_master.log_num_workers;

    let file = File::open(verification_key).unwrap();
    let vk = HyperPlonkVerifyingKey::<E, PCS>::deserialize_compressed(file).unwrap();
